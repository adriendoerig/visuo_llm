...
class HDF5Sequence(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        if self.use_class_weights:
            batch_sample_weights = np.array([self.class_weights[lin] for lin in batch_labels])
        else:
    ...
    def load_dataset(self):
        with h5py.File(self.hdf5_path, "r") as hdf5_file:
            ...
            self.labels = np.empty((self.n_dataset_elements,) + hdf5_file[self.dataset][self.target_dataset_name].shape[1:], 
...
def get_dataset(hparams, dataset, dataset_path=None, plot_generated_data=False):
    ...
    print(f"Making {dataset} dataset from {dataset_path}")
    ...
    tf_dataset = (
        ...
        .map(
            lambda x, y, sw: preprocess_batch(
                x,
                y,
                sw,
                hparams,
