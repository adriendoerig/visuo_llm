...
if GET_WORD_EMBEDDINGS:
    ...
    mean_allWord_embeddings = np.empty((n_nsd_elements, 300))  # fastext embeddings have 300 dimensions
    ...
    for i in range(n_nsd_elements):
        ...
        for w in img_words[i]:
            if w in ms_coco_mean_allWord_embeddings.keys():
                img_allWord_embeddings.append(ms_coco_mean_allWord_embeddings[w])  # we can't read the same embedding twice from the main embeddings dict, so we need to reload pre-existing ones
            else:
                try:
        ...
        mean_allWord_embeddings[i] = np.mean(np.asarray(img_allWord_embeddings), axis=0)

    with open(f"{save_embeddings_to}/nsd_fasttext_ALLWORDS_mean_embeddings.pkl", "wb") as fp:  # Pickling
...
if DO_SANITY_CHECK:
    ...
    with h5py.File(h5_dataset_path, "r") as h5_dataset:
        ...
        with open(f"{save_embeddings_to}/nsd_fasttext_ALLWORDS_mean_embeddings.pkl", "rb") as fp:  # Pickling
        ...
        for i in range(0, total_n_stims, step_size):
            ...
            plt.savefig(f"{save_test_imgs_to}/NSD_allWord_embeddings_check_{i}.png")