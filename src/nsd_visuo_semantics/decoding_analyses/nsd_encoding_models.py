...
for SHUFFLE_DATA in ['random_predictors']:  # if randdom_predictors, 'scramble_embeddings' or 'shuffle_betas_order', we scramble/shuffle the training data before fitting the encoding model
        ...
        for USE_N_STIMULI in [None]:  # None means use all stimuli
            for ROI_NAME in ROI_NAMES:
                ...
                for s_n, subj in enumerate(subs):
                    ...
                    if SHUFFLE_DATA == 'shuffle_betas_order':
                        print("\tWARNING: Shuffling betas_order training data...")
                        print("\tWARNING: Shuffling betas_order training data...")
                        print("\tWARNING: Shuffling betas_order training data...")
                        np.random.shuffle(betas_train)

                    if SHUFFLE_DATA == 'scramble_embeddings':
                        print("\tWARNING: Scrambling embeddings training data...")
                        print("\tWARNING: Scrambling embeddings training data...")
                        print("\tWARNING: Scrambling embeddings training data...")
                        # shuffle the entries in each row of embeddings_train
                        embeddings_train = np.array([np.random.permutation(x) for x in embeddings_train])
                    elif SHUFFLE_DATA == 'random_predictors':
                        print("\tWARNING: Using random_predictors array as training data...")
                        print("\tWARNING: Using random_predictors array as training data...")
                        print("\tWARNING: Using random_predictors array as training data...")
                        # shuffle the entries in each row of embeddings_train
                        embeddings_train = np.random.normal(loc=0, scale=1, size=embeddings_train.shape).astype(np.float32)
