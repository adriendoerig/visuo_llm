...
SPLIT_DATA = 'halves'  # 'halves' or 'full'  # if 'halves', we train two encoders: one on the first half, and the other on the second half of the training set
...
base_save_dir = "/share/klab/adoerig/adoerig/nsd_visuo_semantics/decoding_analyses/results_dir/decoding_analyses"
        ...
        for ROI_NAME in ROI_NAMES:
            if IS_DNN:
                this_results_dir = os.path.join(
                    base_save_dir,
                    f'{ENCODING_MODEL_NAME}_layer{DNN_LAYER}_results_ROI{ROI_NAME}{"" if USE_N_STIMULI is None else f"_{USE_N_STIMULI}stims"}_encodingModel{SPLIT_SUFFIX}',
                )
            else:
                this_results_dir = os.path.join(
                    base_save_dir,
                    f'{ENCODING_MODEL_NAME}_results_ROI{ROI_NAME}{"" if USE_N_STIMULI is None else f"_{USE_N_STIMULI}stims"}_encodingModel{SPLIT_SUFFIX}',
                )
