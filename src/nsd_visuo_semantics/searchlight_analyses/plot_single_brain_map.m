...
fignames = {'clusterAssigmentsVisROIs_random_predictors'};
    ...
    for f = 1:length(fignames)
        ...
        if contains(lower(figname), 'cluster')
            ...
            elseif contains(lower(figname), 'visrois')
                if contains(lower(figname), 'shuffle_betas_order')
                    datapath = '/share/klab/adoerig/adoerig/nsd_visuo_semantics/results_dir/voxel_wise_analyses/cache/encodingModelCoeffs_shuffle_betas_order_visROIs_cluster_assignments_subjavg_nclusters11_zscored.npy'
                    mask_path = ''  % you can provide a path to mask of 0s and 1s. Not used if ''
                elseif contains(lower(figname), 'scramble_embeddings')
                    datapath = '/share/klab/adoerig/adoerig/nsd_visuo_semantics/results_dir/voxel_wise_analyses/cache/encodingModelCoeffs_scramble_embeddings_visROIs_cluster_assignments_subjavg_nclusters11_zscored.npy'
                    mask_path = ''  % you can provide a path to mask of 0s and 1s. Not used if ''
                elseif contains(lower(figname), 'random_predictors')
                    datapath = '/share/klab/adoerig/adoerig/nsd_visuo_semantics/results_dir/voxel_wise_analyses/cache/encodingModelCoeffs_random_predictors_visROIs_cluster_assignments_subjavg_nclusters11_zscored.npy'
                    mask_path = ''  % you can provide a path to mask of 0s and 1s. Not used if ''
                else
        ...
        elseif contains(lower(figname), 'ica')
            if contains(lower(figname), 'highervisrois')
                datapath = '/share/klab/adoerig/adoerig/nsd_visuo_semantics/results_dir/voxel_wise_analyses/cache/encodingModelCoeffs_ica_HigherVisROIs_average_768componentszscore.npy';
                mask_path = ''  % you can provide a path to mask of 0s and 1s. Not used if ''
            end
        else 
            if contains(lower(figname), 'fullbrain')
                ...
                mask_path = ''  % you can provide a path to mask of 0s and 1s. Not used if ''
            elseif contains(lower(figname), 'highervisrois')
                datapath = '/share/klab/adoerig/adoerig/nsd_visuo_semantics/results_dir/voxel_wise_analyses/cache/encodingModelCoeffs_pca3D_HigherVisROIs_average_768componentszscore.npy';
                mask_path = ''  % you can provide a path to mask of 0s and 1s. Not used if ''
            elseif contains(lower(figname), 'ventralrois')
                datapath = '/share/klab/adoerig/adoerig/nsd_visuo_semantics/results_dir/voxel_wise_analyses/cache/encodingModelCoeffs_pca3D_VentralROIs_average_768componentszscore.npy';
                mask_path = ''  % you can provide a path to mask of 0s and 1s. Not used if ''
            elseif contains(lower(figname), 'visrois')
        ...
        for i = 1:length(dims)
            ...
            if contains(lower(figname), 'visroi')
                if strcmp(dim, 'clusters')
                    if plot_single_cluster ~= 0
                        overlay_alpha(data ~= plot_single_cluster) = 0.;
                    else
            ...
            for v = 1:length(viewz_to_plot)
                ...
                if OVERWRITE | ~exist(fullfile(figpath, strcat(figname, 'compo', num2str(dim), 'view', num2str(this_view), '.', SAVE_TYPE)))
                    ...
                    if strcmp(dim, 'clusters')
                        title(sprintf('Cluster %d', plot_single_cluster))
                        suffix = sprintf('clusterID%d', plot_single_cluster);
                    else
                        suffix = ''
                    end
                    saveas(gcf, fullfile(figpath, strcat(figname, 'compo', num2str(dim), 'view', num2str(this_view), suffix)), SAVE_TYPE)
                end
                close all;
            end
        end
