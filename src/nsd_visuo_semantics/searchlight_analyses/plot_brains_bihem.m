...
SAVE_MODEL_NAME = strcat(MODEL_NAME, DNN_LAYER_NAME, MODEL_SUFFIX)
...
% where is the data.
DNN_LAYER = (layer-1)*(DNN_TIMESTEPS(end))+timestep
...
for sub = 1:n_subjects
    ...
    for hemi = 1:2
        ...
        if contains(MODEL_NAME, '_avgSeed')
            ...
            for seed = 1:10
                try
                    seed_data = cat(2, seed_data, readNPY(sprintf(datapath, subj, string(seed), string(seed), this_hemi, subj, string(DNN_LAYER))));
                catch
                    strcat('could not load seed ', string(seed), ' for ', subj, ' hemi ', this_hemi, ' DNN_LAYER ', string(DNN_LAYER))
                end
            end
            avg_seed_data = nanmean(seed_data, 2);
            sub_data = cat(1, sub_data, avg_seed_data);
        else
            sub_data = cat(1, sub_data, readNPY(sprintf(datapath, subj, strcat(MODEL_NAME, MODEL_SUFFIX), this_hemi, subj, string(DNN_LAYER))));
        end
    end
...
CONTRAST_SAVE_MODEL_NAME = strcat(CONTRAST_MODEL_NAME, DNN_CONTRAST_LAYER_NAME, MODEL_SUFFIX)
...
for sub = 1:n_subjects
    ...
    for hemi = 1:2
        ...
        if contains(CONTRAST_MODEL_NAME, '_avgSeed')
            ...
            for seed = 1:10
                try
                    contrast_seed_data = cat(2, contrast_seed_data, readNPY(sprintf(contrast_datapath, subj, string(seed), string(seed), this_hemi, subj, string(contrast_DNN_LAYER))));
                catch
                    strcat('could not load seed ', string(seed), ' for subj ', subj, ' hemi ', this_hemi, ' DNN_LAYER ', string(contrast_DNN_LAYER))
                end
            end
            avg_seed_data = nanmean(contrast_seed_data, 2);
            sub_contrast_data = cat(1, sub_contrast_data, avg_seed_data);
        else
            sub_contrast_data = cat(1, sub_contrast_data, readNPY(sprintf(contrast_datapath, subj, strcat(CONTRAST_MODEL_NAME, MODEL_SUFFIX), this_hemi, subj, string(contrast_DNN_LAYER))));
        end
    end
